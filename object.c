...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    ObjectID computed;
    compute_hash(full_data, file_size, &computed);
    ...
    if (strncmp(full_data, "blob ", 5) == 0) *type_out = OBJ_BLOB;
    else if (strncmp(full_data, "tree ", 5) == 0) *type_out = OBJ_TREE;
    else if (strncmp(full_data, "commit ", 7) == 0) *type_out = OBJ_COMMIT;
    else {
        free(full_data);
        return -1;
    }
    
    *len_out = file_size - (null_pos - full_data + 1);
